...
def export_latest_forecast(out_path: str = 'docs/latest_forecast.json', offline: bool = True) -> str:
    """Run pipeline and write serialized ForecastResult JSON to out_path."""
    print("Running Macro Intelligence Platform forecast export...")
    ...
    if not forecast.validate_schema():
        raise ValueError("Generated forecast payload failed schema validation!")
    ...
    print(f"[+] Successfully exported latest forecast JSON -> {out_path}")
...
if __name__ == '__main__':
    ...
    export_latest_forecast(out_path=args.out, offline=not args.live)
